...
{
	int counter;
	signed long distance;     /* 超音波センサ測定距離[cm] */
	...
	while(1)
	{
		if (++counter == 40/4) /* 約40msec周期処理  */
		{
			/*
			 * 超音波センサによる距離測定周期は、超音波の減衰特性に依存します。NXTの場合は、40msec周期程度が
			 * 経験上の最短測定周期です。（いろいろ試してください)
			 */
			distance = ecrobot_get_sonar_sensor(NXT_PORT_S2); /* 距離測定 */
			counter = 0;
		}

		if (distance <= 30) /* 障害物が30cm以内にあったらライントレース中止 */
		{
			forward = turn = 0;
		}
		else /* ライントレース走行 */
